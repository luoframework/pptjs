...
(function ($) {
  $.fn.pptxToHtml = function (options) {
      ...
      $(this).prepend('<div id="' + domId + '0' + '"></div>')
      var $result = $('#' + domId + '0');
      ...
      //////////////////////////////////////////
      var targetFile = null;                                                               // 文件
      var numOfSlidesTotal = 0;                                                            // 总数
      var numOfSlidesCurrent = 0;                                                          // 当前数
      var numOfSlidesShardSize = options.shardSize ? options.shardSize : 10;               // 区间数
      window.__pptxjs_firstIdent__ = true;                                                 // 首次加载
      var isStraightHan = false;                                                           // 是否正在在处理ppt
      var domIdIndex = 0;                                                                  // 第几个滚动渲染片段容器
      var allPptProgress = 0;                                                              // 进度

      function clearVar (file) {
        targetFile = file;
        numOfSlidesTotal = 0;
        numOfSlidesCurrent = 0;
        numOfSlidesShardSize = options.shardSize ? options.shardSize : 10;
        domIdIndex = 0;
        allPptProgress = 0;

        if (window.__pptxjs_firstIdent__) {
          window.__pptxjs_firstIdent__ = false;
          scrollListen();
        }
      }
      function scrollHan (file) {
        $('#' + domId).append('<div id="' + domId + domIdIndex + '"></div>');
        $result = $('#' + domId + domIdIndex);
        divId = $result.attr("id");
      }

      var toP = 0, toT = 0;
      function scrollListen () {
        $('#' + domId).on("scroll", function (e) {
          if (numOfSlidesCurrent >= numOfSlidesTotal) return;
          if (isStraightHan) return;
          toP = $(this).scrollTop();
          if (toT <= toP) {
            var viewportBottom = toP + $(this).height();
            var scrollHeight = $(this).prop("scrollHeight");
            if (scrollHeight - viewportBottom <= 0) {
              convertToHtml(targetFile, true);
            }
          } else {
            // console.log('向上滚')
          }
          setTimeout(function () { toT = toP; }, 0);
        });
      }
      //////////////////////////////////////////

      function convertToHtml(file, isScroll) {
          if (isStraightHan) return;
          isStraightHan = true;
          if (!isScroll) {
            clearVar(file);
          ...
          // $("#" + divId + " #all_slides_warpper").attr({
          //     style: trnsfrmScl + ";height: " + (numOfSlides * slidesHeight * sScaleVal) + "px"
          // })

          //}

          if (numOfSlidesCurrent < numOfSlidesTotal) {
            $("#" + divId).append(
              $("<div></div>").attr({
                  "class": "slides-loadnig-msg",
                  "style": "display:block; width:100%; color:white; background-color: #ddd;"
                  ...
                  .html($("<div></div>").attr({
                      "class": "slides-loading-progress-bar",
                      "style": "width: " + allPptProgress + "%; background-color: #4775d1;"
                  }).html("<span style='text-align: center;'>Loading... (" + allPptProgress + "%)</span>"))
            );
          }
          domIdIndex++;
          isStraightHan = false;
      ...
      function processPPTX(zip) {
          ...
          numOfSlidesTotal = numOfSlides;
          var temI = numOfSlidesCurrent;
          var temLength = (numOfSlides - temI) < numOfSlidesShardSize ? (numOfSlides - temI) : numOfSlidesShardSize;
          for (var i = temI; i < (temI + temLength); i++) {
